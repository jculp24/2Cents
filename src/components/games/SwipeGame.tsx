...
const SwipeGame = ({ data, onProgress }: SwipeGameProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[600px] relative">
      {/* Indicators */}
      <div className="flex gap-20 absolute top-2 w-full justify-center z-10">
        <motion.div 
      ...
      {/* Item count */}
      <div className="absolute top-12 text-sm text-muted-foreground z-10">
        Card #{(currentIndex % items.length) + 1} of {items.length}
      </div>
      
      {/* Swipe Card - no longer absolute */}
      <motion.div
        style={{ x, rotate }}
        drag="x"
        ...
        className="w-full max-w-xs bg-card shadow-xl rounded-xl cursor-grab active:cursor-grabbing"
      >
      ...
      {/* Button controls - now OUTSIDE the card, with margin-top */}
      <div className="flex gap-12 mt-8">
        <button 
          onClick={() => handleSwipe("left")} 
          className="w-16 h-16 rounded-full bg-red-100 text-red-500 flex items-center justify-center
        ...
        <button 
          ...
          className="w-16 h-16 rounded-full bg-green-100 text-green-500 flex items-center justify-center
